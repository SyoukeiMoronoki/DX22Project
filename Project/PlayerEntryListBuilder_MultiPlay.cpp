...
static PlayerEntry* BuildPlayerEntry(PlayerEntryDataModel* model, T_UINT8* player_id, T_UINT8* computer_id, const std::vector<PlayerCameraViewport>& viewport_list)
{
  ...
  IPlayerCamera* camera = nullptr;
  if (kind == PlayerEntryDataModel::PLAYER_ENTRY_PLAYER)
  {
    const PlayerCameraViewport& viewport = viewport_list[*player_id];
    controller = new PlayerController(*player_id);
    camera = new PlayerCamera(viewport.x, viewport.y, viewport.width, viewport.height);
    (*player_id)++;
  }
...
void PlayerEntryListBuilder_MultiPlay::Build(PlayerEntryDataModelList* models, std::vector<PlayerEntry*>* dest)
{
  ...
  for (T_UINT8 i = 0; i < entries_count; ++i)
  {
    ...
    if (model->GetEntryKind() == PlayerEntryDataModel::PLAYER_ENTRY_PLAYER)
    {
      player_count++;
    }
  }
